...
<jsp:include page="/include/serviceNav.jsp"/>
	...
	<div class="container">
	    ...
	    <tr>
	      <td class="text-left p-0">
	      	<c:if test="${sLevel == 0}"><a href="${ctp}/adminNoticeInput.boss" class="btn btn-outline-warning btn-sm w3-border w3-round-xxlarge font6">글쓰기</a></c:if>
	  ...
	  <table class="table table-hover text-center">
	    ...
	    <c:forEach var="vo" items="${vos}">
		    <tr>
		      <td>${curScrStartNo}</td>
		      <td>
		      	<a href="${ctp}/userNoticeContent.psj?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}&lately=${lately}">${vo.title}</a>
	...
	<div class="container">
		<ul class="pagination justify-content-center">
			...
			<c:if test="${totPage != 0}">
				<c:if test="${pag != 1}">
					<li class="page-item"><a href="userNotice.psj?pag=1&pageSize=${pageSize}&lately=${lately}" class="page-link text-warning" title="첫페이지">◀◀</a></li>
				</c:if>
				<c:if test="${curBlock > 0}">
					<li class="page-item"><a href="userNotice.psj?pag=${(curBlock-1)*blockSize + 1}&pageSize=${pageSize}&lately=${lately}" class="page-link text-warning" title="이전">◀</a></li>
				</c:if>
				<c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize)+blockSize}">
					<c:if test="${i == pag && i <= totPage}">
						<li class="page-item active"><a href='userNotice.psj?pag=${i}&pageSize=${pageSize}&lately=${lately}' class="page-link text-dark bg-warning border-warning">${i}</a></li>
					</c:if>
					<c:if test="${i != pag && i <= totPage}">
						<li class="page-item"><a href='userNotice.psj?pag=${i}&pageSize=${pageSize}&lately=${lately}' class="page-link text-dark">${i}</a></li>
					</c:if>
				</c:forEach>
				<c:if test="${curBlock < lastBlock}">
					<li class="page-item"><a href="userNotice.psj?pag=${(curBlock+1)*blockSize + 1}&pageSize=${pageSize}&lately=${lately}" class="page-link text-warning" title="다음">▶</a></li>
				</c:if>
				<c:if test="${pag != totPage}">
					<li class="page-item"><a href="userNotice.psj?pag=${totPage}&pageSize=${pageSize}&lately=${lately}" class="page-link text-warning" title="마지막">▶▶</a></li>
				</c:if>
			</c:if>
		</ul>
	</div>
