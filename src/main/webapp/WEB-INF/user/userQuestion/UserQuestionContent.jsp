	      ...
	      <tbody>
		      ...
		      <tr class="text-center">
		        <td class="con2" colspan="2">
		          <br>
		            <input class="btn btn-outline-warning" type="button" value="돌아가기" onclick="location.href='${ctp}/userQuestion.psj?pag=${pag}&pageSize=${pageSize}&lately=${lately}';"/>
		        		<c:if test="${sMid == vo.mid}">
		        		<input class="btn btn-outline-warning" type="button" value="수정" onclick="location.href='${ctp}/userQuestionUpdate.psj?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}&lately=${lately}';"/>
		     				</c:if>
			  	...
			  	<tr>
			  		...
			  		<td class="con2" align="left" style="padding-left: 30px;">
			  			<c:if test="${preVO.preIdx != 0}">
							<a href="${ctp}/userQuestionContent.psj?idx=${preVO.preIdx}&pag=${pag}&pageSize=${pageSize}&lately=${lately}">${preVO.preTitle}</a><br/>
							</c:if>
			  		</td>
			  	</tr>
			  	<tr style="border-bottom: 1px solid #E2E2E2;">
			  		...
			  		<td class="con2" align="left" style="padding-left: 30px;">
			  			<c:if test="${nextVO.nextIdx != 0}">
							<a href="${ctp}/userQuestionContent.psj?idx=${nextVO.nextIdx}&pag=${pag}&pageSize=${pageSize}&lately=${lately}">${nextVO.nextTitle}</a><br/>
							</c:if>
			  		</td>
			  	</tr>
	  		</tbody>
		    ...
		    <tbody>
		        ...
		        <td>
			        <c:if test="${sMid == replyVO.mid || sMid == 'admin'}">
							  <a href="${ctp}/userQuestionContent.psj?replyIdx=${replyVO.idx}&idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}&lately=${lately}" class="btn btn-outline-warning btn-sm">수정</a>
					  	</c:if>
							<c:if test="${sMid == replyVO.mid || sMid == 'admin'}">
