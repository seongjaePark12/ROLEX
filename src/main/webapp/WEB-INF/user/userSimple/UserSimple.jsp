...
<head>
  ...
  //부분검색
  	function whatCheck() {
		  ...
		  location.href = "${ctp}/userSimple.psj?pag=${pag}&pageSize=${pageSize}&what="+what;
	  } 
	</script>
</head>
<body>
		...
		<div class="accordion text-left">
			<c:forEach var="vo" items="${vos}" varStatus="st">
				<div class="item"><h6><span style="margin: 0 30px 0 50px;">${st.count}</span><span style="margin-left: 50px;">${vo.title}</span></h6></div>
					<div style="display: none;" class="w3-animate-zoom">
						<br/>
			...
			<div class="container">
				<ul class="pagination justify-content-center">
					...
					<c:if test="${totPage != 0}">
						<c:if test="${pag != 1}">
							<li class="page-item"><a href="userSimple.psj?pag=1&pageSize=${pageSize}" class="page-link text-warning" title="첫페이지">◀◀</a></li>
						</c:if>
						<c:if test="${curBlock > 0}">
							<li class="page-item"><a href="userSimple.psj?pag=${(curBlock-1)*blockSize + 1}&pageSize=${pageSize}" class="page-link text-warning" title="이전">◀</a></li>
						</c:if>
						<c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize)+blockSize}">
							<c:if test="${i == pag && i <= totPage}">
								<li class="page-item active"><a href='userSimple.psj?pag=${i}&pageSize=${pageSize}' class="page-link text-dark bg-warning border-warning">${i}</a></li>
							</c:if>
							<c:if test="${i != pag && i <= totPage}">
								<li class="page-item"><a href='userSimple.psj?pag=${i}&pageSize=${pageSize}' class="page-link text-dark">${i}</a></li>
							</c:if>
						</c:forEach>
						<c:if test="${curBlock < lastBlock}">
							<li class="page-item"><a href="userSimple.psj?pag=${(curBlock+1)*blockSize + 1}&pageSize=${pageSize}" class="page-link text-warning" title="다음">▶</a></li>
						</c:if>
						<c:if test="${pag != totPage}">
							<li class="page-item"><a href="userSimple.psj?pag=${totPage}&pageSize=${pageSize}" class="page-link text-warning" title="마지막">▶▶</a></li>
						</c:if>
					</c:if>
				</ul>
			</div>
