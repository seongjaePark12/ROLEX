...
<script>
    ...
    function midCheck() {
    	...
    	var url = "<%=request.getContextPath()%>/midCheck.psj?mid="+mid;
    	...
    	if(mid=="") {
    		...
    		midCheckOn = 1;
    		window.open(url,"nWin","toolbar=no,location=no,status=no,menuba=no,scrollbars=no,resizable=no,width=500,height=200");
    	}
    }
    ...
    function joinCheck() {
    	...
    	if(mid == "") {
    		...
    		myform.pwd.focus();
   			...
   			myform.pwd.value = "";
/*    		}else if(!pwPattern.test(pwd)){
   			alert("비밀번호는 영문, 숫자, 특수문자만 입력가능합니다.");
   			myform.pwd.value = "";
   			myform.pwd.focus(); */
   		}else if(pwdTest == ""){
    		myform.pwdTest.focus();
 /*   		}else if (!pwPattern.test(pwdTest)) {
   			alert("비밀번호는 영문, 숫자, 특수문자만 입력가능합니다.");
   			myform.pwdTest.value = "";
   			myform.pwdTest.focus(); */
   		}else if(pwd != pwdTest) {
