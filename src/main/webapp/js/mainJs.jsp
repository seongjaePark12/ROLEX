			...
			$(window).scroll(function(){
				...
				$("#btnSearch").click(function(){
					if($("#SearchIcon").hasClass("fa fa-search")){
						...
						var query = {num:num};
						$.ajax({
							url : "/cj2006choihongbok/goods/goodsSearchWord", // 이건 컨트롤로 보낸다
							type : "get",
							data : query,
							success:function(data) {
								$("#searchTitle").html(gender+"인기 검색어");
								for(var i=0; i<5; i++){
									$("#search"+i).html("<li id='search"+i+"'><i class='fa fa-search'></i> <a href='/cj2006choihongbok/goods/goodsSearch?search="+data[i].word+"' style='color: black;'>"+data[i].word+"</a></li>");
								}
							}
						});
			...
			function enterKey(){
				if(window.event.keyCode == 13){
					...
					if(search == ""){
						...
						location.href="/cj2006choihongbok/goods/goodsSearch?search="+search.trim();
					}
				}
			};
	</script>